		...
		listAjax(texts) {
			...
			initDataTokens({
				...
				// 	that.totalCount=res.message.change_wallet.usdt_totle+res.message.lever_wallet.usdt_totle+res.message.micro_wallet.usdt_totle+res.message.legal_wallet.usdt_totle;
					for(let i=0;i<res.message.change_wallet.balance.length;i++){
					    that.totalCount+=res.message.change_wallet.balance[i].change_balance*1+res.message.change_wallet.balance[i].lock_change_balance*1
					}
					for(let i=0;i<res.message.micro_wallet.balance.length;i++){
					    that.totalCount+=res.message.micro_wallet.balance[i].micro_balance*1+res.message.micro_wallet.balance[i].lock_micro_balance*1
					}
					for(let i=0;i<res.message.lever_wallet.balance.length;i++){
					    that.totalCount+=res.message.lever_wallet.balance[i].lever_balance*1+res.message.lever_wallet.balance[i].lock_lever_balance*1
					}
					for(let i=0;i<res.message.legal_wallet.balance.length;i++){
					    that.totalCount+=res.message.legal_wallet.balance[i].legal_balance*1+res.message.legal_wallet.balance[i].lock_legal_balance*1
					}
					if (that.type == 'lever') {
